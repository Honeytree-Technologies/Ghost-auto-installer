...
"######################################################################"
"#                                                                    #"
"#            Ghost Installation and Hardening Script                 #"
"#                                                                    #"
"#              Created by Honeytree Technologies, LLC                #"
"#                        www.honeytreetech.com                       #"
"#                                                                    #"
"#                    Ghost: honeytree.social                         #"
...
"#####                                                              #####"
"#####  This will install Ghost on fresh server.                    #####"
"#####                                                              #####"
"##### Installing on an operating Ghost server will wipe data.      #####"
"#####                                                              #####"
"########################################################################"
startup_warning
sleep 3
...
while true; do
    read -p "Enter a password: " admin_password
    if [ ${#admin_password} -lt 10 ]; then
        echo "Password must be at least 10 characters long."
        continue
    fi

    if ! [[ "$admin_password" =~ [[:upper:]] && "$admin_password" =~ [0-9] && "$admin_password" =~ [^a-zA-Z0-9] ]]; then
        echo "Password must contain at least one uppercase letter, one digit, and one special symbol."
        continue
    fi
    break
done
...
while true; do
  ...
  if [ -n "$site_title" ]; then
    ...
    echo "site title name cannot be empty. Please enter site title."
  fi
done
...
read -p "Enter the DB user name (Default: ghost): " db_user
if [ -z "${db_user}" ]; then
  db_user=ghost
fi
...
while true; do
  ...
  if [ -n "$db_root_password" ]; then
    ...
    echo "DB_ROOT_PASSWORD cannot be empty. Please enter db root password."
  fi
done
...
temp_db="ghost_$(random_string 8)"
...
while true; do
  read -p "Enter SMTP TRANSPORT: " smtp_transport
  if [ -n "$smtp_transport" ]; then
    break
  else
    echo "SMTP TRANSPORT cannot be empty. Please enter smtp transport."
  fi
done
...
services:
  ghost:
    ...
    environment:
      ...
      mail__transport: ${smtp_transport}
      mail__options__host: ${smtp_host}
      mail__options__port: ${smtp_port}
      mail__options__auth__user: ${smtp_user}
      mail__options__auth__pass: ${smtp_password}
      mail__from: ${smtp_from_address}
...
docker compose -f "${work_dir}/docker-compose.yml" exec ghost apt update
docker compose -f "${work_dir}/docker-compose.yml" exec ghost apt install wget -y

sleep 20

docker compose -f "${work_dir}/docker-compose.yml" exec -it ghost wget \
  --header="Content-Type: application/json; charset=UTF-8" \
  --header="Accept: application/json, text/javascript, /; q=0.01" \
  --header="Host: localhost:2368" \
  --header="Origin: http://localhost:2368" \
  --header="Referer: http://localhost:2368/ghost/" \
  --header="Connection: keep-alive" \
  --header="X-Requested-With: XMLHttpRequest" \
  --header="App-Pragma: no-cache" \
  --post-data='{"setup":[{"name":"'"${admin_user}"'","email":"'"${admin_email}"'","password":"'"${admin_password}"'","blogTitle":"'"${site_title}"'"}]}' \
  "http://localhost:2368/ghost/api/v3/admin/authentication/setup/"
...
cat <<nginx_content >>/etc/nginx/sites-available/ghost
server {
...
echo "Admin email:  ${admin_email}, Username: ${admin_user}  and  password: ${admin_password}"
